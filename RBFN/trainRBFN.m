function [Centers, betas, Theta, X_activ] = trainRBFN(X_train, Y_train, centersPerCategory, verbose,training_type)
...
%         end
    ...
    Centers = [X_train(randi(size(X_train,1)),:)];
     for k =1: centersPerCategory * numCats - 1
        flag = 0;
        while(flag == 0)
            
            random_center = X_train(randi(size(X_train,1)),:);
            for j = 1:size(Centers,1)
                if size(find(random_center == Centers(j,:)),2) == size(Centers,2); 
                    break;
                end
                if j == size(Centers,1)
                    flag = 1;
